...
public class BikeControllerEasy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Debug.DrawRay(transform.position, laterialForward);
        ...
        if (pedalSteer != 0.0f) {
            // Bike Tilt
            transform.localRotation = Quaternion.AngleAxis(-pedalSteer * Time.deltaTime / 10.0f , transform.forward) * transform.localRotation;
            // Bike Rotation
            transform.rotation = Quaternion.AngleAxis(pedalSteer * Time.deltaTime * degreesPerCycle, Vector3.up) * transform.rotation;
        ...
        m_Animator.SetFloat("anim_speed", Mathf.Max(1.0f, rb.velocity.magnitude/4.0f));
    }
